const { createLayout, Template } = require('./runtime')

function build(tree, state, options) {
  return createLayout(tree).call(new Template(), state, options)
}
...
testData.forEach(d => {
  ...
  if (!deepEqual(actual, d.expected)) {
    throw new Error('not equal: ' + JSON.stringify(actual) + ' != ' + JSON.stringify(d.expected))
  }
})
